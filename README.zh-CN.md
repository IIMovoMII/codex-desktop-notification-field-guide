...
~~~text
Codex，请阅读 https://github.com/IIMovoMII/codex-desktop-notification-field-guide，先只读检查我这台 Windows 电脑上的 Codex Desktop 版本、可用钩子、JSONL 事件结构、进程生命周期以及是否已经安装 CC Connect，然后在部署前必须先问我要让 CC Connect 连接哪个通讯平台，至少向我列出 QQ 官方机器人、Telegram、飞书／Lark、个人微信和当前版本检测到的其他平台，并明确说明个人微信受主动发送条数和会话限制影响，不推荐用于无人值守提醒，再根据我的选择和本机实际情况构建一个通过 CC Connect 发送、随正常启动的 Codex Desktop 一起启动和退出的本地提醒程序，能够识别正常完成、结构化和未知错误、用户暂停、等待批准与等待输入，不把长时间安静误报成卡死，过滤 CLI 与子代理任务，把提示词和对话正文留在本机，不允许聊天消息启动 Codex CLI，使用增量文件监听以及可持久化、去重、重试的发送队列保证消息可靠，在安装 CC Connect 或写入当前钩子前先说明影响并获得我的确认，最后用合成事件覆盖状态竞争，并通过我选定的平台完成一次真实的单向通知验证。
~~~
...
**为什么不推荐个人微信：** 本项目来源环境曾实测到大约累计十条未回复提醒后停止发送，需要用户重新互动；CC Connect 上游也记录过约十条附近出现长回复中断（[问题 #770](https://github.com/chenhg5/cc-connect/issues/770)）、服务端会话限制（[问题 #1087](https://github.com/chenhg5/cc-connect/issues/1087)），以及实测每账号约 24 小时只能主动发送五到六条，后来默认将安全预算设为四条（[已合并 PR #1643](https://github.com/chenhg5/cc-connect/pull/1643)）。具体数字可能随平台变化，但这种限制本质上不适合可靠的无人值守通知。
