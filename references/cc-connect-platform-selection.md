...
[CC Connect](https://github.com/chenhg5/cc-connect)是本指南固定的消息桥，但具体通讯平台由用户决定。只读盘点后、安装或绑定前，把平台、专用／共享实例和是否需要入站交互放在同一组需求问题里询问。

## 必问问题

> 你希望把 Codex Desktop 提醒发到哪个 CC Connect 平台？要使用专用还是共享 CC Connect？只接收提醒，还是还需要受控的入站交互？
...
6. 按用户选择配置“仅提醒”或独立保护的入站控制；不能从旧配置偷偷决定。
7. 发送一条合成通知，要求用户确认可见。
