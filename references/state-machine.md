...
`Stop` 到来时进入“收尾中”，不立刻断言完成。由于同事件命令钩子并发运行，另一个 `Stop` 钩子仍可能要求继续；`stop_hook_active` 也不能证明所有并发决定已收敛。等待当前版本观察到的后续活动和末尾追加：

- 出现明确失败，转“已失败”；
- 出现用户中断，转“用户中断”；
- 出现独立的明确完成记录，转“已完成”；
- 出现任何新活动，撤销本次 `Stop` 候选并回到“运行中”；
- 只有 `Stop` 而 Desktop 仍在运行时，继续观察，不靠固定延时猜完成；
- 观察必须结束但仍没有独立终态时，转“需要检查”，并在诊断中说明能力缺口。
...
终态通知使用跨重启稳定的事件键，例如“任务＋轮次＋本次终态决定”，不能把标题、摘要文字或后来补充的错误描述混入键中。同一事件在入队前重新分类时更新原队列项，不新增消息；投递后是否发送一条带相同关联编号的更正，由用户选择。等待通知键另含请求编号。

每个会话只保留当前未完成轮次；新轮次出现时，旧的未完成轮次静默退役，不能在 Desktop 退出时一起扇出异常提醒。状态先持久化、通知再入队，两个动作使用同一事务或可恢复顺序；已投递事件账本必须跨观察程序重启保留。
