<div align="center">

# Codex Desktop 消息提醒经验指南

**给编程智能体的一份实战说明书：把 Codex Desktop 的任务状态可靠地转成 CC Connect 消息提醒。**

[![经验指南](https://img.shields.io/badge/%E7%B1%BB%E5%9E%8B-%E7%BB%8F%E9%AA%8C%E6%8C%87%E5%8D%97-6f42c1)](#这是什么)
[![适配](https://img.shields.io/badge/%E9%80%82%E9%85%8D-%E6%8C%89%E6%9C%AC%E6%9C%BA%E7%8E%AF%E5%A2%83-0078D4)](#适用范围)
[![架构](https://img.shields.io/badge/%E6%9E%B6%E6%9E%84-%E4%BA%8B%E4%BB%B6%E9%A9%B1%E5%8A%A8-0f766e)](#总体结构)
[![开源许可](https://img.shields.io/badge/%E8%AE%B8%E5%8F%AF-MIT-16a34a)](LICENSE)

[English](README.en.md) · [给智能体的入口](SKILL.md) · [配套的账号切换指南](https://github.com/IIMovoMII/codex-auth-switching-field-guide)

</div>

## 这是什么

这个仓库**不是现成的机器人程序，也不是预先配置好的消息桥**。它是一份主要写给编程智能体看的产品说明书和实战经验：根据当前电脑上的 Codex Desktop 版本、事件格式和用户需求，设计适合本机的提醒程序。消息桥固定使用 [CC Connect](https://github.com/chenhg5/cc-connect)，但具体通讯平台和是否需要入站交互由用户选择。

> 这里提供的是产品目标、状态判断经验和验收方法，不绑定某一种源码。Windows 做法是已经验证过的参考，其他系统可以换成自己的等价实现。

它解决的是一个很实际的需求：用户用自己习惯的任何方式打开 Codex，留下一个长任务，然后在任务完成、报错、被暂停或需要操作时收到提醒。提醒程序不应依赖某个专用快捷方式或包装启动命令。

## 具体使用场景

| 你的情况 | 针对本机编写的方案应该做到 |
| --- | --- |
| Codex Desktop 长任务运行时人不在电脑旁 | 只在任务真正完成或需要处理时发送一条有用提醒 |
| API 或中转经常出现服务、网络、认证等不同报错 | 优先保留结构化错误类别，无法分类时仍发通用错误提醒，并可靠重试消息 |
| 用户在任务中途主动暂停 | 把明确的用户中断与失败、正常完成区分开 |
| Codex 自己停止、重试耗尽或自动暂停 | 有明确系统停止证据时单独提醒；证据不够时落到“需要检查”，不冒充用户暂停 |
| 任务正在等待批准或补充输入 | 每个不同请求只提醒一次，恢复运行后不继续重复 |
| 用户可能从桌面、开始菜单、命令、文件关联或其他入口打开 Codex | 使用与入口无关的生命周期钩子或系统进程事件，让观察程序自动跟随 Desktop 启停 |
| 同一台电脑会切换官方 OAuth 与 API 档案 | 保留钩子和监控状态，并用维护标记避免计划内重启被误报 |
| 以后可能更换通讯平台 | 状态识别不依赖某个平台，但所有出站提醒都统一经过 CC Connect |
| 正常长任务几个小时没有本地事件 | 继续观察，但绝不根据安静时长发送“疑似卡死” |

这套方案的核心是本地任务提醒。如果还想通过聊天软件控制电脑，可以把入站权限、确认和审计做成独立的可选功能。

## 一句话部署

把下面这句话复制到一个新的 Codex 任务中：

~~~text
Codex，请完整阅读 https://github.com/IIMovoMII/codex-desktop-notification-field-guide（从 `SKILL.md` 开始），结合我的本机环境和需求，为我实现并验证一个通过 CC Connect 发送的 Codex Desktop 提醒程序。
~~~

这里的“部署”是把任务一次性交给 Codex，不是安装预先配置的机器人。如果缺少必要的通讯平台、账号或权限，Codex 应只询问必须由你选择的内容，并始终让凭据留在对话之外。

## 可以按需求裁剪

最简单可以只保留“完成＋报错”两种提醒；需要时再加入等待批准、等待输入、状态页、多平台或安全的入站交互。部署时只要说明想接收哪些状态、消息包含多少内容、使用哪个通讯平台、是否复用现有 CC Connect，以及希望打开 Codex 就启动还是等首个任务再启动；系统、路径和事件格式可以直接从电脑检查。

无论选择哪些功能，都应保证状态有证据、消息不过度泄露内容、重复提醒能合并、通讯失败后不会直接丢失结果。

## 为什么不能只看一个信号

钩子很快，但用途不同。当前[官方钩子说明](https://learn.chatgpt.com/docs/hooks)明确：同一事件的多个命令钩子会并发启动，所以一个 `Stop` 钩子看到“准备停止”时，另一个 `Stop` 钩子仍可能要求继续。`Stop` 只能提供收尾候选，不能单独宣布完成。`SessionEnd` 可能在关闭、归档、删除或较长空闲后才触发，而且始终同步执行；它也不能承担每轮完成提醒。JSONL 证据更丰富，但反复读取全部文件太浪费。进程退出只说明应用不在了，不能单独判断任务为何结束。

可靠方案需要把几个小信号组合起来，而不是相信某一个万能事件。

## 总体结构

~~~mermaid
flowchart LR
    H[Codex 钩子] --> N[本地事件整理]
    J[JSONL 增量读取] --> N
    P[Desktop 进程状态] --> N
    N --> S[每个任务独立的状态机]
    S --> D[去重与隐私裁剪]
    D --> O[持久发送队列]
    O --> A[CC Connect 发送器]
    A --> Q[用户选定的 CC Connect 平台]
~~~

钩子负责快速唤醒。操作系统原生的文件变化通知在历史追加时唤醒增量读取器；Windows 可使用目录变化通知，其他系统换成等价机制。低频校准只负责防止漏事件，不扫描所有对话，也不根据运行时长判断任务失败。状态机内部不依赖具体平台，但本指南支持的发送路线最终必须进入 CC Connect。

## 哪些情况应该提醒

| 状态 | 证据要求 | 是否提醒 |
| --- | --- | --- |
| 正常完成 | 明确的完成事件或经过验证的结束状态 | 是 |
| 结构化错误 | 明确的错误类型、状态或编号 | 是 |
| 未知错误 | 已确认失败，但暂时无法分类 | 是，使用通用错误名称 |
| 用户暂停／中断 | 明确的中断证据 | 是 |
| 系统停止／自动暂停 | 明确的非用户停止证据 | 是；无法证明原因时改为“需要检查” |
| 等待批准 | 明确的批准请求 | 是 |
| 等待输入 | 明确的输入请求，或当前版本已验证的等价事件 | 是 |
| Desktop 退出但结果未知 | 有活动任务、无维护标记，收尾后仍无终态 | 是，提示“需要检查”，不称为失败 |
| 长时间安静 | 没有任何终止证据 | 否 |
| 计划内重启 | 其他本地工具写入了短期维护标记 | 不发崩溃提醒 |
| 观察系统异常 | 钩子未信任、解析器不兼容、队列或消息桥故障 | 在本机显示；能投递时另发健康提醒 |

不要只靠计时器生成“疑似卡死”。Codex 的长任务可能正常运行几个小时。可以监控提醒程序自身是否健康，但不要用这一套逻辑给任务下结论。

## 十条核心原则

1. **直接观察 Desktop。** 不要求用户从包装脚本启动，也不绑定某个图标或入口。
2. **给钩子明确分工。** `SessionStart` 负责唤醒，`Stop` 只提供完成候选，`SessionEnd` 只做同步清理。
3. **只读取文件新增部分。** 为每个文件保存游标，用当前系统的文件事件唤醒。
4. **尽早过滤来源。** 排除 CLI、子代理和提醒程序自己产生的任务，避免循环。
5. **按任务记录状态变化。** 完成、失败、暂停、等待和“需要检查”都是可收敛状态，不是几个关键词。
6. **优先读结构化错误。** 文字匹配只补充版本差异；无法分类的错误也应提醒。
7. **默认只发必要内容。** 完整提示词和对话留在本机；只有用户主动选择时，才加入脱敏且限长的回复摘要。
8. **先入队，后发送。** 通讯平台确认成功后，才能从队列移除。
9. **固定消息桥，再选择平台。** 所有提醒统一通过 CC Connect，状态判断不依赖用户最终选中的 CC Connect 平台。
10. **计划内维护要协同。** 短期标记只抑制计划内退出，不能掩盖明确错误。

## 阅读路线

| 文档 | 用途 |
| --- | --- |
| [信号检查](references/signal-discovery.md) | 检查钩子、JSONL、进程和来源标记 |
| [状态机](references/state-machine.md) | 组合信号并处理竞争，不制造计时误报 |
| [增量监听](references/incremental-monitoring.md) | 低开销监听文件变化并保存读取位置 |
| [CC Connect 平台选择](references/cc-connect-platform-selection.md) | 询问用户具体连接哪个平台，并说明为什么不推荐个人微信 |
| [消息投递](references/delivery.md) | 设计持久队列、重试、CC Connect 发送器和可选入站边界 |
| [隐私](references/privacy.md) | 决定哪些内容可以离开本机 |
| [验证](references/validation.md) | 测试状态竞争、重启和通讯平台故障 |

## 任意方式启动 Codex 都能跟随

无论从桌面、开始菜单、命令、文件关联还是系统恢复这些正常入口打开 Codex，都不需要专用快捷方式或包装脚本。默认方案可以在第一个 Codex 生命周期事件出现时按需启动，并在 Desktop 退出、末尾事件和待发消息处理完后自动结束。

如果希望“Codex 窗口一出现，提醒程序就已经就绪”，部署时选择进程级联动即可。两种方案的资源占用和启动时机不同，具体实现见[增量监听](references/incremental-monitoring.md)。

## 选择 CC Connect 平台

本指南必须使用 CC Connect。部署时由你明确选择要连接的平台，不会仅凭电脑上碰巧存在的旧配置自动决定。

| 选择 | 实际建议 |
| --- | --- |
| QQ 官方机器人 | 用户主要使用 QQ 且能完成开发者认证时优先选择；上游使用官方接口，无需公网 IP |
| Telegram | 网络可以访问 Telegram 时适合使用；上游采用长轮询，无需公网 IP |
| 飞书／Lark | 适合稳定的个人或团队流程；上游使用 WebSocket，无需公网 IP |
| QQ OneBot | 可以使用，但要额外部署 NapCat 等第三方 OneBot，风险与 QQ 官方机器人不同 |
| 个人微信 iLink | **不推荐用于无人值守提醒** |
| 企业微信或其他受支持平台 | 根据用户已有账号和当前 CC Connect 版本再决定 |

**为什么不推荐个人微信：** CC Connect 上游已经记录过个人微信主动发送预算和限流问题，具体数字还会随账号、版本和服务端变化。无人值守提醒最怕“程序发了，但平台没送到”，因此默认更适合选择 QQ 官方机器人、Telegram 或飞书。证据和版本差异见[平台选择说明](references/cc-connect-platform-selection.md)。

个人微信与企业微信是 CC Connect 中两个不同的平台。上面的警告针对个人微信 iLink，企业微信应单独评估，不能混为一谈。

先配置一个用户选定的平台并完成一次真实出站验证，确认稳定后再考虑增加第二个平台。

你可以选择“仅接收提醒”或“也允许入站交互”。仅提醒模式可以忽略普通消息；如果需要入站控制，应把它作为独立模块，明确允许的动作、发送者、确认方式、审计与限速，不能让任意消息直接变成本机命令。

## 实战中最容易踩的坑

- `SessionEnd` 官方定义就是同步执行，而且不是每轮完成钩子；把联网发送或后台启动塞进去会拖慢退出并造成误判。
- 钩子定义变更可能触发重新审查；未信任的钩子不会因为文件存在就自动工作。
- 完成和用户中断事件可能先后竞争，需要很短的收尾窗口和明确的优先级。
- 进程消失不代表当前任务一定失败，但收尾后仍没有证据时必须通知“结果未知，需要检查”。
- 看似已经停止后，JSONL 仍可能追加末尾记录。
- 通知通常只需要任务标题；很多时候只读 SQLite 标题即可，没必要读取正文。
- 每秒轮询整棵文件树既浪费磁盘，也不可靠；文件变化通知加增量游标更合适。
- 持久队列能保证通讯平台短暂故障时不丢结果。
- 消息桥支持很多平台，不代表每个平台都适合提醒；个人微信的主动发送限制使它不应成为默认方案。
- `Stop` 事件可能与其他并发 `Stop` 钩子的续跑决定竞争，必须等独立终态证据，不能靠固定延时直接宣布完成。
- 去重应使用跨重启稳定的事件编号，不能把标题、摘要文字或后来补充的错误说明算成新事件；同一任务旧轮次也要静默退役，避免一次退出扇出多条异常提醒。
- 账号/API 切换器只有坚持补丁式修改并保留钩子时，提醒程序才会跨模式继续工作；切换后仍要跑一条真实通知。

## 适用范围

本指南面向 Codex Desktop 本地提醒，方法可迁移到不同系统和单向／双向场景；现有实测主要来自 Windows。其他平台应替换文件事件、进程观察、凭据存储和自启动机制并重新验证。钩子事件、进程名称和 JSONL 结构都可能随版本变化，Codex 升级后应重新检查并运行测试。

## 安全

通讯平台凭据、用户编号、真实对话和本地绝对路径都不能进入仓库。监控程序应在本地裁剪错误，只发送完成判断所需的最少内容。详见 [SECURITY.md](SECURITY.md)。

## 参与改进

欢迎补充可复现的信号、状态竞争、测试样例和可靠投递方法。请同时注明 Codex 版本范围和验证方式。详见 [CONTRIBUTING.md](CONTRIBUTING.md)。

## 许可

[MIT](LICENSE)。通讯平台规则、额度与凭据安全仍由实际操作者负责。
